...
local command_transform = require("system.transform.transform_command")
...
function M:onAddToWorld()
	self.world.transform = command_transform.create(self)
...
function M.event_merge_policy(event, events)
	...
	for i = 1, #events do
		...
		if e.entity == entity then
			...
			e.delay = event.delay or e.delay
			e.callback = event.callback or e.callback
			return true
		end
	end
